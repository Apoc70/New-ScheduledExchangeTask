<# 
    ...
    1.2 Changes to task argument list
...
function Check-Exchange {
  ...
  {
    Write-Error -Message 'Exchange Server environment variable not present. Check your Exchange Server setup.'
...
function New-ScheduledTaskFolder {
  ...
  Write-Verbose -Message 'Checking scheduled task folder (group)'
...
function Create-AndRegisterExchangeTask {
  ...
  $taskArgument = ("-version 3.0 -command "". '{0}'; Connect-ExchangeServer -auto; {1}" -f $ExchangeScriptPath, $ExchangeTaskScript)

  # Create task action
  $taskAction = New-ScheduledTaskAction -Execute ('{0}\powershell.exe' -f $PSHome) -Argument $taskArgument
  ...
  if($ExchangeTaskDescription -eq '') {
    ...
    Write-Verbose -Message ('No description provided, setting ExchangeTaskDescription to: {0}' -f ($ExchangeTaskDescription))
  }

  Write-Verbose -Message 'Registering task'
...
# Fetch Major and Minor OS Version String
